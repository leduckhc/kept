...
import { type Thread, syncInbox, loadThreads, loadSnoozedThreads, loadStarredThreads, loadRepliedToSenders, markRead, archiveThread, blockSender, fetchMessageBody, sendEmail, groupBySection, hasSyncedBefore } from './gmail';
...
// helpers, toasts, avatar imported above
...
function renderCommandPalette() {
  ...
  const commands: PaletteCommand[] = [
    ...
    // Compose
    { id: 'compose',       label: 'Compose New',       shortcut: 'c', icon: '✏', group: 'Compose', action: () => openComposeNew() },
    // Navigate
    { id: 'go-inbox',      label: 'Go to Inbox',       icon: '✉', group: 'Navigate', action: () => switchView('Inbox') },
    { id: 'go-snoozed',    label: 'Go to Snoozed',     icon: '🕐', group: 'Navigate', action: () => switchView('Snoozed') },
    { id: 'go-sent',       label: 'Go to Sent',        icon: '↗', group: 'Navigate', action: () => switchView('Sent') },
    { id: 'go-drafts',     label: 'Go to Drafts',      icon: '✏', group: 'Navigate', action: () => switchView('Drafts') },
    { id: 'go-starred',    label: 'Go to Starred',     icon: '★', group: 'Navigate', action: () => switchView('Starred') },
    // App
  ...
  function renderList(q: string) {
    ...
    if (searchMode) {
      ...
      searchResults.forEach((t, i) => {
        ...
        item.addEventListener('click', () => { close(); openThread(t); });
    ...
    if (!q) {
      // Show recents first, then all grouped
