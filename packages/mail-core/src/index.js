...
export function groupInboxThreads(threads, { now = new Date() } = {}) {
    ...
    return Object.fromEntries(inboxSectionDefinitions.map((section) => [
        section.title,
...
export function createPkceVerifier({ cryptoImpl = globalThis.crypto, byteLength = 64 } = {}) {
    if (!cryptoImpl?.getRandomValues)
        throw new Error('Web Crypto getRandomValues is required for Gmail PKCE');
...
export async function createPkceChallenge(verifier, { cryptoImpl = globalThis.crypto } = {}) {
    ...
    if (!cryptoImpl?.subtle?.digest)
        throw new Error('Web Crypto subtle.digest is required for Gmail PKCE');
...
export function parseGmailOAuthCallback(callbackUrl, { expectedState } = {}) {
    ...
    if (error)
        return { ok: false, state, error, errorDescription: parsed.searchParams.get('error_description') || '' };
    const code = parsed.searchParams.get('code');
    if (!code)
        throw new Error('Gmail OAuth callback missing authorization code');
...
export function createKeychainTokenStore({ keychain, service = gmailOAuthTokenKeyPrefix } = {}) {
    ...
    ['setPassword', 'getPassword', 'deletePassword'].forEach((method) => {
        if (typeof keychain[method] !== 'function')
            throw new Error(`keychain.${method} is required`);
    });
    return {
...
export function createGmailApiConnector({ tokenStore, fetchImpl = globalThis.fetch, accountId = 'acct_gmail_primary', tokenUrl = 'https://oauth2.googleapis.com/token', clientId = '', clientSecret = '', now = () => new Date(), } = {}) {
...
export function createJsonMailStore({ storage, key = 'kept.gmail.sync.v1' } = {}) {
    ...
    ['getItem', 'setItem', 'removeItem'].forEach((method) => {
        if (typeof storage[method] !== 'function')
            throw new Error(`storage.${method} is required`);
    });
    return {
...
export async function syncGmailInbox({ connector, accountId, mailStore, repository, accountEmail, maxResults = 25 } = {}) {
    ...
    try {
        ...
        if (repository)
            await saveGmailSyncToRepository({ repository, accountId, accountEmail, connector, result: nextResult });
        if (mailStore)
            await mailStore.saveSyncResult(nextResult);
...
function redactStructuredValue(value) {
    ...
    return Object.fromEntries(Object.entries(value).map(([key, nested]) => {
        if (/body|raw|payload|snippet|prompt|messages|responseBody|response_body|response/i.test(key))
            return [key, '[body-redacted]'];
        if (/token|secret|codeVerifier|code_verifier|authorization_code|code$/i.test(key))
...
async function readAccessToken(tokenStore, accountId, refreshOptions = {}) {
    ...
    if (!tokens?.accessToken)
        throw new Error('Gmail access token not found in keychain');
    if (isTokenExpired(tokens, refreshOptions.now) && tokens.refreshToken) {
        ...
        if (typeof tokenStore.saveTokens === 'function')
            await tokenStore.saveTokens(accountId, nextTokens);
...
async function refreshGmailAccessToken(tokens, { fetchImpl, tokenUrl, clientId, clientSecret, now = () => new Date() } = {}) {
    ...
    if (!response?.ok)
        throw createGmailAuthRevokedError(`Gmail token refresh failed with ${response?.status || 'unknown'} status`);
...
async function fetchGmailJson(fetchImpl, url, accessToken) {
    ...
    if (!response?.ok)
        throw new Error(`Gmail API request failed with ${response?.status || 'unknown'} status`);
...
async function modifyGmailMessageLabels({ tokenStore, accountId, fetchImpl, tokenUrl, clientId, clientSecret, now, messageId, payload }) {
    ...
    if (response?.status === 401 || response?.status === 403) {
        if (typeof tokenStore.clearTokens === 'function')
            await tokenStore.clearTokens(accountId);
        throw createGmailAuthRevokedError(`Gmail credentials were revoked or expired with ${response.status} status`);
    }
    if (!response?.ok)
        throw new Error(`Gmail API modify failed with ${response?.status || 'unknown'} status`);
    return response.json();
}
...
function parseMboxHeaders(headerLines) {
    ...
    return Object.fromEntries(unfolded
        .map((line) => {
...
function parseMailbox(value) {
    ...
    if (!match)
        return { name: decoded, email: decoded.includes('@') ? decoded : '' };
...
export function getLocalMailSearchState({ enabled = true, indexing = false, query = '', resultCount = null, index = null, repositoryUpdatedAt = null, error = null } = {}) {
    if (!enabled)
        return { status: 'disabled', label: 'Search disabled', detail: 'Local search is unavailable until mail storage is enabled.' };
    if (error)
        return { status: 'error', label: 'Search error', detail: userSafeSearchError(error) };
    if (indexing)
        return { status: 'indexing', label: 'Indexing local mail', detail: 'Kept is preparing local mail search on this device.' };
    ...
    const api = {
        ...
        async setFlags(messageId, flags) {
            ...
            if (!state.messages[messageId])
                throw new Error(`Local message not found: ${messageId}`);
        ...
        async updateTriageActionStatus(actionId, patch = {}) {
            ...
            if (!state.triageActions[actionId])
                throw new Error(`Triage action not found: ${actionId}`);
...
async function loadLocalMailState(storePath, initialData, bodyEncryption) {
    if (initialData)
        return migrateLocalMailState(initialData, bodyEncryption);
    try {
        ...
        if (error?.code === 'ENOENT')
            return emptyLocalMailState();
        if (error?.code === 'KEPT_LOCAL_STORE_CORRUPT')
            throw error;
...
function migrateLocalMailState(raw, bodyEncryption) {
    if (!raw || typeof raw !== 'object')
        throw createRepositoryCorruptionError('local store root is not an object');
...
function serializeLocalMailStateForDisk(state, bodyEncryption) {
    ...
    diskState.messages = Object.fromEntries(Object.entries(diskState.messages).map(([id, message]) => {
...
function deserializeLocalMessageFromDisk(message, bodyEncryption) {
    if (!message || typeof message !== 'object')
        return message;
    if (!message.bodyCiphertext && !message.snippetCiphertext)
        return message;
    return {
        ...message,
...
async function serializeBrowserLocalMailStateForDisk(state, bodyEncryption) {
    const diskState = clone(state);
    diskState.messages = Object.fromEntries(await Promise.all(Object.entries(diskState.messages).map(async ([id, message]) => {
        const encrypted = { ...message };
...
async function createBrowserBodyEncryptionContext({ key, bodyEncryptionKey, cryptoImpl }) {
    ...
    if (!subtle || !cryptoImpl?.getRandomValues)
        throw new Error('Web Crypto is required for browser local mail repository encryption');
...
function decryptRepositoryText(value, bodyEncryption) {
    ...
    if (version !== 'v1' || !ivBase64 || !tagBase64 || !ciphertextBase64)
        throw createRepositoryCorruptionError('invalid encrypted body payload');
    try {
...
async function encryptBrowserRepositoryText(value, bodyEncryption) {
    ...
    const ciphertext = await bodyEncryption.subtle.encrypt({ name: 'AES-GCM', iv }, bodyEncryption.key, new TextEncoder().encode(String(value || '')));
...
async function decryptBrowserRepositoryText(value, bodyEncryption) {
    ...
    if (version !== 'web-v1' || !ivBase64 || !ciphertextBase64)
        throw createRepositoryCorruptionError('invalid encrypted body payload');
    try {
        const plaintext = await bodyEncryption.subtle.decrypt({ name: 'AES-GCM', iv: base64ToBytes(ivBase64) }, bodyEncryption.key, base64ToBytes(ciphertextBase64));
...
function maxIsoTimestamp(left, right) {
    ...
    if (Number.isNaN(leftValue))
        return right || left || new Date(0).toISOString();
...
function normalizeTriageActionEntry(entry = {}) {
    ...
    if (!gmailTriageActionTypes.includes(entry.action))
        throw new Error(`Unsupported Gmail triage action: ${entry.action}`);
    const status = entry.status || 'queued';
    if (!gmailTriageActionStatuses.includes(status))
        throw new Error(`Unsupported triage action status: ${status}`);
...
function normalizePartialLocalFlags(flags = {}) {
    const next = {};
    if (Object.hasOwn(flags, 'read'))
        next.read = Boolean(flags.read);
    if (Object.hasOwn(flags, 'starred'))
        next.starred = Boolean(flags.starred);
    if (Object.hasOwn(flags, 'archived'))
        next.archived = Boolean(flags.archived);
    return next;
}
...
function sanitizeSecretFields(value) {
    if (Array.isArray(value))
        return value.map(sanitizeSecretFields);
    if (!value || typeof value !== 'object')
        return value;
